...
class ExplainAndAskNode:
    def __init__(self, tools: BasicTools) -> None:
        self.tools = tools

    async def __call__(self, state: AgentState) -> AgentState:
        logger.info("entering node=explain_and_ask")
        result = await self.tools.generate_action_response(state, action="explain_and_ask")
        return {
            ...
            "workflow_trace": append_trace(state, "explain_and_ask"),
        }
